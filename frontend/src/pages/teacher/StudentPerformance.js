...
import { getAllStudents, getStudentPerformance } from '../../api/teacher';
import {
  BarChart2, Star, Zap, BookOpen, TrendingUp,
  ChevronDown, ChevronUp, Hash, User, CircleDashed,
} from 'lucide-react';
...
function levelProg(pts) {
  ...
  const pct  = next
    ...
    : 100;
  return { pct, nextLevel: next, remaining: next ? LEVEL_THRESH[next] - pts : 0 };
}
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mt-0.5">
            <span className="text-xs text-slate-400 flex items-center gap-1"><Hash size={10} />Roll {student.roll_no}</span>
            <span className="text-xs text-slate-400 flex items-center gap-1"><User size={10} />Age {student.age}</span>
      ...
      <div className="animate-fade-in">
        <div className="mb-6">
          ...
          <p className="text-slate-500 mt-1">
            {students.length} student{students.length !== 1 ? 's' : ''} � Click any student to see their detailed progress
          </p>
        </div>
        ...
        {students.length === 0 ? (
          <div className="card text-center py-16">
            ...
            <p className="text-slate-500">No active students in your classrooms yet.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {students.map((s) => <StudentRow key={s.student_id} student={s} />)}
          </div>
        )}
      </div>
    </DashboardLayout>
